...
// Format D: Generic fallback with flexible date/time separator
const WA_GENERIC = /^[\d./-]{6,10},?\s+[\d:]{4,8}(?:\s*[AP]M)?\s*[-–]\s*(.+?):\s([\s\S]*)/i;
...
function parseWhatsApp(lines) {
  ...
  for (const line of lines) {
    ...
    if (match) {
      ...
      if (match.length === 3) {
        // Generic pattern (2 capture groups: sender, text)
        let [, sender, text] = match;
        // Strip WhatsApp broadcast ~ prefix
...
function parseInstagram(lines) {
  ...
  while (i < lines.length) {
    ...
    // Look ahead: is next line a timestamp?
    const nextLine = lines[i + 1] ? lines[i + 1].trim() : '';
    const timeMatch = IG_SENDER_TIME.exec(nextLine) || /^\d{1,2}:\d{2}\s*(AM|PM)/i.exec(nextLine);

    if (timeMatch && line.length <= 40 && !DATE_LIKE.test(line)) {
      // Current line = sender, next = time, after = message
      const sender  = line;
      ...
      if (msgLine && msgLine.length > 0) {
        ...
        baseTime += Math.random() * 300000 + 30000;
        i += 3;
        continue;
      }
    }
